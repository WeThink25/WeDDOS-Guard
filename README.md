...
1. **Download the Script:**
   Save the provided script as `weddos_guard.py` in a directory of your choice (e.g., `/opt/weddos/`).
...
3. **Configure the Script:**
   Open `weddos_guard.py` in a text editor and adjust the CONFIG section:
...
- **Log to File:** Redirect output:
  ```bash
  sudo /path/to/weddos_guard.py > /var/log/weddos.log 2>&1
  ```

### Running as a System Service
To run automatically on boot:
1. Create a systemd service file at `/etc/systemd/system/weddos-guard.service`:
   ```ini
   [Unit]
...
3. Check status:
   ```bash
   sudo systemctl status weddos-guard
   ```
...
- **Firewall Errors:** Ensure `iptables`/`ipset` modules are loaded:
  ```bash
  sudo modprobe ip_set xt_set
  ```
- **Remove Rules:** To clear all WeDDOS rules and sets:
  ```bash
  sudo iptables -D INPUT -j WEDDOS-PORT
